...
function [cumDistance, timeStep] = distance_estimate(file, measuringAlgo)
...
%     figure();
    ...
    if measuringAlgo == "weinberg"
        if exist('steps','var')
            ...
            stepLength = (Amax - Amin)^0.25 * 0.365;
            %stepLength = (Amax - Amin)^0.25 * 0.33
            j = 1;
            currentDistance = 0;
            for i = 1:length(t)
        ...
        elseif measuringAlgo == "integrate"
            cumDistance = cumtrapz(cumtrapz(accelX));
        end
